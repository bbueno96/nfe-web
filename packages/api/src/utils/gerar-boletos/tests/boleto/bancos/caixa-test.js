...
const Caixa = require('../../../lib/boleto/bancos/caixa.js')
const geradorDeLinhaDigitavel = require('../../../lib/boleto/gerador-de-linha-digitavel.js')

const Datas = boletos.Datas
const Endereco = boletos.Endereco
const Beneficiario = boletos.Beneficiario
const Pagador = boletos.Pagador
const Boleto = boletos.Boleto
...
const boletos = require('../../../lib/utils/functions/boletoUtils.js')
...
module.exports = {
  ...
  'Nosso número formatado deve ter 17 digitos'(test) {
  ...
  'Carteira formatado deve ter dois dígitos'(test) {
    const beneficiario = Beneficiario.novoBeneficiario().comCarteira('1')
    const numeroFormatado = banco.getCarteiraFormatado(beneficiario)
  ...
  'Conta corrente formatada deve ter cinco dígitos'(test) {
  ...
  'Linha digitavel com carteira SIGCB 1'(test) {
    const datas2 = Datas.novasDatas()
    datas2.comDocumento('02-04-2020')
    datas2.comProcessamento('02-04-2020')
    datas2.comVencimento('02-04-2020')
    ...
    const linhaEsperada = '10496.48999 58000.100048 00000.000711 7 81550000015876'

    test.equal(linhaEsperada, geradorDeLinhaDigitavel(codigoDeBarras, banco))
    test.done()
  },

  'Linha digitavel com carteira SIGCB 2'(test) {
    const codigoDeBarras = banco.geraCodigoDeBarrasPara(boletoSicgb)
    const linhaEsperada = '10492.90271 45900.200044 00000.013227 5 81550000008000'
  ...
  'Verifica a numeração correta do banco'(test) {
    test.equal(banco.getNumeroFormatadoComDigito(), '104-0')
    test.done()
  },

  'Verifica que arquivo de imagem do logotipo existe'(test) {
  ...
  'Exibir campo CIP retorna falso'(test) {
    test.equal(banco.exibirCampoCip(), false)
    test.done()
  },

  'Verifica criação de pdf - SIGCB 1'(test) {
    new PdfGerador(boletoSicgb).pdfFile('../tests/boleto/bancos/boleto-caixa1.pdf').then(async ({ path }) => {
  ...
  async 'Verifica criação de pdf - SIGCB 2'(test) {
    new PdfGerador(boletoSicgb).pdfFile('../tests/boleto/bancos/boleto-caixa2.pdf').then(async ({ path }) => {
      test.ok(fs.existsSync(path))
      test.equal(fs.unlinkSync(path), undefined)
      test.done()
    })
