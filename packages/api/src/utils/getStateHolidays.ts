...
const getStateHolidays = (year, uf) => {
  const holidaysByStateCode = [
    {
      state: 'SP',
      date: parseISO(`${String(year)}-07-09`),
      description:
    ...
    {
      state: 'AL',
      date: parseISO(`${String(year)}-06-29`),
      description: 'São Pedro - Lei estadual nº 5.509/1993',
    },
    { state: 'AL', date: parseISO(`${String(year)}-09-16`), description: 'Emancipação política' },
    {
      state: 'AL',
      date: parseISO(`${String(year)}-11-20`),
    ...
    {
      state: 'DF',
      date: parseISO(`${String(year)}-11-30`),
      description: 'Dia do evangélico - Lei distrital nº 963/1995',
    },
    {
      state: 'AM',
      date: parseISO(`${String(year)}-09-05`),
    ...
    {
      state: 'AP',
      date: parseISO(`${String(year)}-09-13`),
      description: 'Criação do Território Federal (Data Magna do estado) - Art. 335 da Constituição estadual',
    },
    {
      state: 'SC',
      date: parseISO(`${String(year)}-08-11`),
      description: 'Dia de Santa Catarina (criação da capitania, separando-se de São Paulo)',
    },
    {
      state: 'SC',
      date: parseISO(`${String(year)}-11-25`),
      description: 'Dia de Santa Catarina de Alexandria',
    },
    { state: 'AC', date: parseISO(`${String(year)}-01-23`), description: 'Dia do evangélico' },
    {
      state: 'AC',
      date: parseISO(`${String(year)}-03-08`),
      description: 'Alusivo ao Dia Internacional da Mulher',
    },
    { state: 'AC', date: parseISO(`${String(year)}-06-15`), description: 'Aniversário do estado' },
    { state: 'AC', date: parseISO(`${String(year)}-09-05`), description: 'Dia da Amazônia' },
    {
      state: 'AC',
      date: parseISO(`${String(year)}-11-17`),
      description: 'Assinatura do Tratado de Petrópolis',
    },
  ]
  return holidaysByStateCode.filter(holiday => holiday.state === ('' + uf).toUpperCase())
}
export function isStateHoliday(givenDate: Date, stateCode: string) {
  const stateHolidays = getStateHolidays(givenDate.getFullYear(), stateCode)
  for (const holiday of stateHolidays) {
    // if (holiday.date.isSame(givenDate, 'day')) {
    if (format(holiday.date, 'dd.MM.yyyy') === format(givenDate, 'dd.MM.yyyy')) {
      return true
    }
  }
  return false
