  ...
  if (id && parameters && parameters.nfeCnpj && parameters.passwordCert && parameters.pfx) {
    ...
    const xmls = notas.map(nota => {
      ...
      if (tipoFrete !== 9) {
        transporta = {
          ...
          CPF: (nota.transpCpfCnpj?.length || 0) === 11 ? nota.transpCpfCnpj : null,
      ...
      const det = products
        .map((product, i) => {
          ...
          else if (totalitem === (nota.qtdeProdutos || 0))
          ...
          if (totalFrete === (nota.frete || 0)) freteArred = 0
          else {
          ...
          if (totalOutro === nota.outrasDespesas) freteArred = 0
            ...
            if (totalOutro > (nota.outrasDespesas || 0))
              ...
              outroArred = parseFloat(vOutros) + ((nota.outrasDespesas || 0) - (totalOutro || 0))
          }
          ...
          return removeEmpty({
            ...
            prod: {
              ...
              vUnTrib: isExternal ? (priceArred / (pesoproduto * (quantity || 0)))?.toFixed(4) : unitPrice?.toFixed(4),
    ...
    try {
      if (requestData.length < 500000 && nfeId !== '') {
        ...
        if (Number(envioResponse.cStat) === 103 || Number(envioResponse.cStat) === 104) {
          for (const nota of notas) {
