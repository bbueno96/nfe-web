...
export class ExportBradesco400UseCase {
  ...
  async execute(data: IExportBradesco400DTO) {
    ...
    if (bankAccount && parameters && parameters.nfeRazao) {
      bankAccount.sequenceLot = bankAccount?.sequenceLot || 1 + 1
      ...
      for (let i = 0; i < data.installments.length; i++) {
        ...
        if (installment) {
          ...
          try {
            ...
            fileData += `${parseFloat((installment?.interest || 0).toString()) > 0 ? 2 : 0}` // 10.1 Campo de Multa Se = 2 considerar percentual de multa. Se = 0, sem multa  (tam: 1) 066-066
            fileData += `${parseFloat((installment.interest || 0).toString())
              .toFixed(2)
              .replace('.', '')}`.padStart(4, '0') // 11.1  Percentual de Multa (tam: 4) 067-070
            ...
            fileData += `${parseFloat((installment.value || 0).toString())
              .toFixed(2)
              .replace('.', '')}`.padStart(13, '0') // 23.1 Valor do Título  (tam: 13) 127-139
            ...
            fileData += `${(installment.fine?.div(100).times(installment.value || 0)
              ? parseFloat((installment.value || 0).toString())
              : 0
            )
              .toFixed(2)
      ...
      fileData += `\r\n`
      sequential += 1
      await this.bankAccountRepository.update(bankAccount.id, bankAccount, null)
