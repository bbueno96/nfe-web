...
export class ExportSicredi240UseCase {
  ...
  async execute(data: IExportSicredi240DTO) {
    ...
    if (bankAccount && parameters && parameters.nfeRazao && parameters.nfeCnpj) {
      ...
      for (let i = 0; i < data.installments.length; i++) {
        ...
        let customer
        if (parameters.nfeLagradouro && inst) {
          customer = {
        ...
        fileData += `${parseFloat((installment.value || 0).toString())
          .toFixed(2)
          .replace('.', '')}`.padStart(15, '0') // 21.3P Valor (tam: 13) 086-100
        ...
        fileData += `${parseFloat((installment.interest || 0).toString())
          .toFixed(2)
          .replace('.', '')}`.padStart(15, '0') // 28.3P Juros  (tam: 13 ) 217-141
        ...
        fileData += `${parseFloat((installment.fine || 0).toString())
          .toFixed(2)
          .replace('.', '')}`.padStart(15, '0') // 16.3R Multa (tam: 13,  ) 075-089
