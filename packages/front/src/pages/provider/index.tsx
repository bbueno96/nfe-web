...
export const ProviderList = () => {
  ...
  const { fetching, items, pager, setFetching, query, updateData, updatePage, updatePerPage, updateSort } =
    usePostList<ProviderListValues>({ initialQuery: { sort: [{ name: 'id' }] } })
  useEffect(() => {
    setFetching(true)

    axios
      .post('provider.list', query, { headers: { Authorization: `Bearer ${saToken}` } })
...
export const ProviderForm = () => {
  ...
  function handleSubmit(values: ProviderListValues) {
    if (id) {
      axios
